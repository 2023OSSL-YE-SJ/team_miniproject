...
int main(){
    ...
    while (1) {
        if (mode == 1) { // 사용자모드
            menu = userMenu();
            if (menu == 0) break;
            if (menu == 1) { // 메뉴 선택 + 칼로리 검색
                // 메뉴의 종류를 보여주고 선택하게 한다
                int productMenu = selectProductMenu();
                int productNumber = selectProduct(sp, productMenu);
                // 메뉴의 종류를 고르면 해당 종류의 모든 메뉴를 보여준다
                // 칼로리를 고를지 선택하게 한다
                // 메뉴 종류와 카테고리를 기준으로 해당하는 메뉴만 보여준다
                if (productNumber == 0) { // 칼로리 선택
                    // 칼로리 입력받기
                    int calorie = getCalorie();
                    selectProductByCategoryAndCalorie(sp, productMenu, calorie);
                }
                printf("주문이 완료되었습니다.\n");
                break;
            } else if (menu == 6) {
        ...
        } else if (mode == 2) { // 관리자모드
            menu = adminMenu();
            if (menu == 0) {
                break;
            } else if (menu == 1) { // 메뉴 추가
